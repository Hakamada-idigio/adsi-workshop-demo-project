...
import { useApproveCorrection, useRejectCorrection } from "./useCorrections";
...
export function ApprovalActions({ correctionId, version }: ApprovalActionsProps) {
  ...
  function handleApprove() {
    approveMutation.mutate({ id: correctionId, version });
  }
  ...
  function handleReject(e: FormEvent) {
    ...
    rejectMutation.mutate(
      { id: correctionId, reason: rejectReason.trim(), version },
      {
        onSuccess: () => {
          setOpen(false);
          setRejectReason("");
  ...
  return (
    <div className="flex gap-2">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle>修正申請を却下</DialogTitle>
